...
calc.ISE <- function(n, no_var, true_rel, dis, true_var, essentially,
  ...
  est_alpha <- cronbach(x)
  x.mean <- rowMeans(x)
  est_sig <- sqrt(var(x.mean) * (1 - est_alpha))
  out <- f.tau(x.mean, est_sig, rot = FALSE)
  
  # evaluate ISE by approximated functions
  est.f <- approxfun(out$w, Re(out$eval))
  SE <- function(x) return((est.f(x) - true.f(x)) ^ 2)
  ISE <- integrate(SE, min(out$w), max(out$w), subdivisions = 100000)$value
...
# )
# write.csv(sim_results_1000, "sim_results_1000_deconvolve.csv")
# gc()
# mpi.exit()
# stopCluster(cl)
